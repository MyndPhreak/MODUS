...
export class ServerStatusService {
    ...
    private async checkServers() {
        ...
        const servers = allServers.filter(server => this.client.guilds.cache.has(server.$id));

        if (servers.length === 0) return;

        console.log(`[ServerStatusService] [Shard ${this.client.shard?.ids[0] ?? 0}] Checking ${servers.length} relevant servers (out of ${allServers.length} total)...`);
        ...
        for (const server of servers) {
            try {
                ...
                await this.appwriteService.updateServerStatus(server.$id, isOnline, ping, shardId);
            } catch (error) {
                console.error(`[ServerStatusService] Error checking server ${server.name}:`, error);
            }
        }
    }
