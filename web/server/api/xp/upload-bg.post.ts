/**
 * Upload an XP rank card background image to R2.
 *
 * Stores under `xp/<guild_id>/<rand>.<ext>`. Returns a stable proxy
 * URL `/api/xp/bg/<key>` that the editor persists into the template.
 */
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    console.error("[XP API] R2 upload failed:", error?.message || error);
    throw createError({
      statusCode: 500,
